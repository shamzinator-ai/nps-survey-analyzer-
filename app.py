...
if file and validate_file(file):
    ...
    analysis_mode = st.selectbox(
        ...
        [
            ...
            "Free Text Only",
            "Both Structured and Free Text",
        ],
        index=2,
        help="Choose which parts of the pipeline to run when processing data.",
    )
    ...
    if st.button(
        ...
        run_free_text = analysis_mode != "Structured Data Only"
        run_structured = analysis_mode != "Free Text Only"

        if run_free_text:
            with st.spinner("Processing free-text responses..."):
                processed_subset = process_free_text(
                    df_to_process, free_text_cols, cache_path
                )
            ...
            processed_subset = df_to_process
